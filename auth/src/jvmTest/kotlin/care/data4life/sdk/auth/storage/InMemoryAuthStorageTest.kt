...
package care.data4life.sdk.auth.storage
...
class InMemoryAuthStorageTest {
    ...
    fun fullInMemoryStorageTestCase() {
        ...
        assertFalse(actualInitialContains, "Initial storage should be empty")

        // store AuthState
        sut.writeAuthState(ALIAS, AUTH_STATE)

        val actualShouldContain = sut.containsAuthState(ALIAS)
        assertTrue(actualShouldContain, "AuthState should be present for the alias")

        // Read AuthState
        val actualAuthState = sut.readAuthState(ALIAS)
        assertEquals(AUTH_STATE, actualAuthState, "AuthStates should match")

        // remove AuthState
        sut.removeAuthState(ALIAS)
        val actualContainAfterRemoval = sut.containsAuthState(ALIAS)
        assertFalse(actualContainAfterRemoval, "AuthState should be removed")
